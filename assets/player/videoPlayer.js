...
Video.fn.createSkipAd = function(){
	...
	this.skipAdBoxContentLeft.append('<p class="elite_vp_skipAdTitle">' + "Reklamı Geç" + '</p>');
...
Video.fn.createVideoAdTitleInsideAD = function(){
	...
	this.videoAdBoxInside.append('<p class="elite_vp_adsTitleInside">' + "Reklam" + '</p>');
...
{
	var self=this;
	$(this.embedWindow).append('<p class="elite_vp_embedTitle elite_vp_themeColorText elite_vp_titles">' + "PAYLAŞ" + '</p>');
	$(this.embedWindow).append('<p class="elite_vp_embedTitle2 elite_vp_themeColorText elite_vp_titles">' + "İFRAME KODU:" + '</p>');
	...
	*/
	...
	$(this.embedWindow).append('<p class="elite_vp_embedTitle3 elite_vp_themeColorText elite_vp_titles">' + "VİDEO PAYLAŞ LİNKİ:" + '</p>');
...
Video.fn.openAD = function()
{
	...
	{
		$(".elite_vp_skipAdCountTitle").text("Reklamı Geçmek için " + (self._playlist.videos_array[self._playlist.videoid].prerollSkipTimer) + " s");
		this.i.src = self._playlist.videos_array[self._playlist.videoid].thumbnail_image;
	}
};
...
Video.fn.playVideoById = function(ID)
{
	...
	else if(self._playlist.videos_array[ID].videoType=="youtube" || self.options.videoType=="YouTube")
	{
		...
		if(self._playlist.youtubePlayer!= undefined){
			...
			{
				self._playlist.youtubePlayer.cueVideoById(self._playlist.videos_array[ID].youtubeID);
			}
		}
		self.resizeAll();
	}
...
Video.fn.setupEvents = function()
{
	...
	$(".elite_vp_videoPlayerAD").bind("loadeddata", function() {
		...
		self.myInterval = setInterval(function () {
			...
			$(".elite_vp_skipAdCountTitle").text("Reklamı Geçmek İçin " + self.counter + " s");
			if(self.counter==0 )
			{
				self.toggleSkipAdCount();
